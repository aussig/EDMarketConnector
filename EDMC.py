...
sys.path: {sys.path}"""
        ...
        if args.j:
            ...
            try:
                ...
                logfiles = sorted(
                    (x for x in os.listdir(monitor.currentdir) if JOURNAL_RE.search(x)), key=lambda x: x.split(".")[1:]
                )
        ...
        if not deep_get(data, "commander", "name", default="").strip():
            ...
            or ((data["commander"]["docked"] and data["lastStarport"]["name"] or None) != monitor.station)
